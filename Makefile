...
${FIPTOOL}: FORCE
ifdef UNIX_MK
	${Q}${MAKE} CPPFLAGS="-DVERSION='\"${VERSION_STRING}\"'" FIPTOOL=${FIPTOOL} OPENSSL_DIR=${OPENSSL_DIR} --no-print-directory HOSTCC=$(HOST_COMPILE)gcc -C ${FIPTOOLPATH}
else
...
BOOTPARAMPATH ?= bootparameters_src
.PHONY: bootparam
bootparam:
	${Q}${MAKE} CPPFLAGS="-DVERSION='\"${VERSION_STRING}\"'" GCC=$(HOST_COMPILE)gcc -C ${BOOTPARAMPATH} all
